...
const data = (connection, name, dbVersionSupport, consistencyLevel, tenant) => {
  ...
  return {
    ...
    insertMany: (objects) =>
      connection.batch(name, consistencyLevel).then((batch) =>
        __awaiter(void 0, void 0, void 0, function* () {
          const serialized = yield index_js_3.Serialize.batchObjects(name, objects, tenant);
    ...
    replace: (obj) =>
      objectsPath
        .buildUpdate(obj.id, name, consistencyLevel)
        .then((path) =>
          connection.put(path, Object.assign({ class: name, tenant: tenant }, parseObject(obj)))
        ),
    update: (obj) =>
      objectsPath
        .buildUpdate(obj.id, name, consistencyLevel)
        .then((path) =>
          connection.patch(path, Object.assign({ class: name, tenant: tenant }, parseObject(obj)))
        ),
  };
};
